...
[ ] Add an option to command `rag` so that `rag --update` compares the latest commit and the commit in store.json by their full SHAs. For all changed files, it remove all chunks of those files from the local vector store and add new chunks from the new file version. You can search all chunks of those files using the `file_path` field.
